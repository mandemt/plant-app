...
const registerWatch = [{getData: "SELECT * FROM plants  AS planten JOIN profile AS p ON p.plant_id = planten.plant_id WHERE planten.plant_id=?"},{postData: "UPDATE profile SET watched=? WHERE plant_id = ?"}]

      db.query(registerWatch[0].getData, id, (err, data) => {
    let result = data
    postData(Number(result[0].watched))
